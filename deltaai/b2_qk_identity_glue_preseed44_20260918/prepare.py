...
assert manifest['attention_type'] == 'gt_mha_identity'
assert (manifest['num_base_heads'], manifest['num_generators']) == (2, 8)
print('PASS: selected seed-44 QK identity checkpoint hashes and manifest', flush=True)
...
replace_bert_self_attention(model, attention_type='gt_mha_identity', num_base_heads=2,
...
assert sum(p.numel() for p in model.parameters()) == 92190906
...
classifier, audit = load_bert_sequence_classifier(str(CHECKPOINT), num_labels=2,
    attention_type='gt_mha_identity', num_base_heads=2, num_generators=8,
...
for layer in classifier.bert.encoder.layer:
    ...
    assert attention.value_generators.requires_grad and attention.value_theta.requires_grad
    ...
    for key in ('eval_matthews_correlation', 'eval_combined_score', 'eval_accuracy'):
        for lr, score in [('2em5', .8), ('3em5', .8), ('5em5', .7)]:
            for seed in (42, 43, 44):
                out = root / f'test_gt_mha_identity_lr{lr}_seed{seed}_grid'
                out.mkdir(exist_ok=True)
                (out / 'eval_results.json').write_text(json.dumps({key: score}))
        sys.argv = [str(selector), '--root', str(root), '--prefix', 'test', '--metric-key', key,
            '--methods', 'gt_mha_identity', '--output-json', str(root/'result.json'), '--output-tsv', str(root/'result.tsv')]
        runpy.run_path(str(selector), run_name='__main__')
        assert json.loads((root/'result.json').read_text())['gt_mha_identity']['selected_learning_rate'] == '2e-5'
(STAGE/'preflight.json').write_text(json.dumps(dict(status='passed', pretrained_seed=44,
    parameters=92190906, exact_backbone_restoration=True, glue_cache=cache,

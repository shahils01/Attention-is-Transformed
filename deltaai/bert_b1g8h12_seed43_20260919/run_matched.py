"""Validate the historical BERT recipe, then launch its one-base ablation."""
import json
import os
...
REPO = STAGE / "repo"
REFERENCE = Path('/work/hdd/bicn/anayak2/lgma_runs/bert_base_gt_mha_residual_b4g8h12_random_seed42_explicit_fuseqkv_v2')
RUN_NAME = 'bert_base_gt_mha_residual_b1g8h12_random_seed43_explicit_20260919'
OUTPUT = REFERENCE.parent / RUN_NAME
reference = json.loads((STAGE / 'reference_manifest.json').read_text())['arguments']
requested = dict(reference)
requested.update(seed=43, num_base_heads=1, enforce_paper_gt_mha=False,
                 output_dir=str(OUTPUT), run_name=RUN_NAME)
assert {k for k in requested if requested[k] != reference[k]} == {'seed', 'num_base_heads', 'enforce_paper_gt_mha', 'output_dir', 'run_name'}
assert requested['seed'] == 43
argv = []
...
model, audit = load_bert_masked_lm(
    ...
    initialization=args.initialization, num_base_heads=1, num_generators=8,
...
for layer in model.bert.encoder.layer:
    ...
    assert (attn.num_base_heads, attn.num_heads, attn.num_generators, attn.base_dim, attn.value_dim) == (1, 12, 8, 64, 64)
    assert attn.theta_init == 'random_sphere' and attn.theta_init_scale == 0.02
    assert attn.generator_init_scale == 0.02 and attn.metric_mode == 'residual'
...
training_args = TrainingArguments(output_dir='/tmp/bert-b1-preflight-unused', use_cpu=True, report_to=[],
...
for layer in model.bert.encoder.layer:
    attn = layer.attention.self.gt_attention
    for p in (attn.generators, attn.theta, attn.value_generators, attn.value_theta):

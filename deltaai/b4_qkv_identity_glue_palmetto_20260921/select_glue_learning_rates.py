...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Select one GLUE learning rate per attention type by mean dev score."
    )
    parser.add_argument("--root", type=Path, required=True)
    parser.add_argument("--prefix", required=True)
    parser.add_argument("--metric-key", required=True)
    parser.add_argument(
        "--methods", nargs="+", default=("mha", "gqa", "collaborative", "gt_mha_identity_both")
    )
    ...
    for method in args.methods:
        ...
        for learning_rate in args.learning_rates:
            ...
            for seed in args.seeds:
                ...
                if args.metric_key not in metrics:
                    raise KeyError(f"{args.metric_key!r} missing from {metrics_path}")
                scores.append(float(metrics[args.metric_key]))

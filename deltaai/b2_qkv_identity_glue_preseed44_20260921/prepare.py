...
replace_bert_self_attention(model, attention_type='gt_mha_identity_both', num_base_heads=2,
...
assert sum(p.numel() for p in model.parameters()) == 91796538
...
classifier, audit = load_bert_sequence_classifier(str(CHECKPOINT), num_labels=2,
    attention_type='gt_mha_identity_both', num_base_heads=2, num_generators=8,
...
(STAGE/'preflight.json').write_text(json.dumps(dict(status='passed', pretrained_seed=44,
    parameters=91796538, exact_backbone_restoration=True, glue_cache=cache,

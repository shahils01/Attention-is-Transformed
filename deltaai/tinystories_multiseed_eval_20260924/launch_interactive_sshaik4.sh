...
echo "Requesting sshaik4 interactive GH200 allocation"
salloc --account=biad-dtai-gh --partition=ghx4 --nodes=1 --ntasks=1 \
  --gpus-per-node=1 --cpus-per-task=4 --mem=32G --time=02:00:00 \
  srun --ntasks=1 --pty bash -lc "bash $ROOT/run_interactive_sshaik4.sh"

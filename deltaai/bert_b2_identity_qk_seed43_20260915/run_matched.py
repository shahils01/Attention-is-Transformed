...
RUN_NAME = 'bert_base_gt_mha_identity_qk_b2g8h12_random_seed43_explicit_20260915'
OUTPUT = Path('/work/hdd/bicn/anayak2/lgma_runs') / RUN_NAME
reference = json.loads((STAGE / 'reference_manifest.json').read_text())['arguments']
requested = dict(reference)
requested.update(seed=43, num_base_heads=2, attention_type="gt_mha_identity", enforce_paper_gt_mha=False,
...
for layer in model.bert.encoder.layer:
    ...
    assert attn.value_transform_mode == 'residual'
...
for layer in model.bert.encoder.layer:
    ...
    for p in (attn.value_generators, attn.value_theta):
...
assert {k for k in requested if requested[k] != baseline['arguments'][k]} == {'attention_type', 'output_dir', 'run_name', 'seed'}
assert bert_parameter_counts(model)['model_parameter_count'] == 92190906

...
assert manifest['attention_type'] == 'gt_mha_identity_both'
assert (manifest['num_base_heads'], manifest['num_generators']) == (4, 8)
print('PASS: selected seed-44 QKV identity checkpoint hashes and manifest', flush=True)
...
replace_bert_self_attention(model, attention_type='gt_mha_identity_both', num_base_heads=4,
    num_generators=8, generator_mixing='softmax', use_sdpa=True,
    fuse_base_qkv=True, sdpa_gqa_mode='native', initialize_from_mha=False,
    enforce_paper_gt_mha=False)
load_model(model, str(CHECKPOINT / 'model.safetensors'), strict=True)
assert sum(p.numel() for p in model.parameters()) == 95340090
...
classifier, audit = load_bert_sequence_classifier(str(CHECKPOINT), num_labels=2,
    attention_type='gt_mha_identity_both', num_base_heads=4, num_generators=8,
    fuse_base_qkv=True, enforce_paper_gt_mha=False)
assert len(audit) == 12
for layer in classifier.bert.encoder.layer:
    attention = layer.attention.self.gt_attention
    assert attention.metric_mode == 'identity'
    assert attention.value_transform_mode == 'none'
    assert not hasattr(attention, 'theta') and not hasattr(attention, 'generators')
    assert not hasattr(attention, 'value_generators') and not hasattr(attention, 'value_theta')
    ...
    for key in ('eval_matthews_correlation', 'eval_combined_score', 'eval_accuracy'):
        for lr, score in [('2em5', .8), ('3em5', .8), ('5em5', .7)]:
            for seed in (42, 43, 44):
                out = root / f'test_gt_mha_identity_both_lr{lr}_seed{seed}_grid'
                out.mkdir(exist_ok=True)
                (out / 'eval_results.json').write_text(json.dumps({key: score}))
        sys.argv = [str(selector), '--root', str(root), '--prefix', 'test', '--metric-key', key,
            '--methods', 'gt_mha_identity_both', '--output-json', str(root/'result.json'), '--output-tsv', str(root/'result.tsv')]
        runpy.run_path(str(selector), run_name='__main__')
        assert json.loads((root/'result.json').read_text())['gt_mha_identity_both']['selected_learning_rate'] == '2e-5'
(STAGE/'preflight.json').write_text(json.dumps(dict(status='passed', pretrained_seed=44,
    parameters=95340090, exact_backbone_restoration=True, glue_cache=cache,

...
def build_attention(
    ...
    if attention_type in LGMA_ATTENTION_TYPES:
        ...
        if attention_type == "gt_mha_exact":
        ...
        elif attention_type in {"lgma_residual", "lgma_qkv_identity"}:
            metric_mode = "residual"
            logit_scale_mode = "rms_metric"
            learn_head_temperature = True
            ...
            if num_base_heads == 1:
                num_base_heads = 2
        attention_class = QKVIdentityAttention if attention_type == "lgma_qkv_identity" else LieGeneratedMetricAttention

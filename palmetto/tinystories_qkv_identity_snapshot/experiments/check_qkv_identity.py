...
from lgma.identity import QKVIdentityAttention
from lgma.synthetic import CharTokenizer, make_lm_batch
from ospool.train_tinystories_compat import compact_encode
from experiments.train_tinystories_qkv_identity import compact_batch as compact_make_lm_batch
...
torch.manual_seed(0)
model = TinyTransformerLM(attention_type="lgma_qkv_identity", **kw)
for name, parameter in model.named_parameters():
    assert torch.equal(parameter, dict(ref.named_parameters())[name]), name
for m in model.modules():
    if isinstance(m, QKVIdentityAttention):
        ...
        assert torch.equal(m.compute_value_transforms(), torch.eye(4).expand(8,4,4))
inputs = torch.randint(0,23,(2,8))
# Independent reference: zero score/value generators in the residual model.
with torch.no_grad():
    for m in ref.modules():
        if hasattr(m, "generators"):
            m.generators.zero_()
            m.value_generators.zero_()
    torch.testing.assert_close(model(inputs), ref(inputs), rtol=1e-6, atol=1e-7)
result = model(inputs)
...
for m in model.modules():
    if isinstance(m,QKVIdentityAttention):
        assert "value_generators" not in dict(m.named_parameters())
        assert "value_theta" not in dict(m.named_parameters())
        assert torch.equal(m.compute_value_transforms(), torch.eye(4).expand(8,4,4))
...
buffer.seek(0)
restored = TinyTransformerLM(attention_type="lgma_qkv_identity", **kw)
...
print("PASS: retained initialization, identity invariance, fixed QK/V transforms and residual-reference equivalence, checkpoint roundtrip, compact sampler parity", flush=True)

import sys
from experiments.train_tinystories_qkv_identity import install
training = install()
sys.argv = [__file__, "--data_path", __file__, "--device", "cpu",
            ...
            "--value_transform", "lie", "--attention", "lgma_qkv_identity",

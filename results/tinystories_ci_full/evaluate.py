...
ids=valid
manifest={'seed':17029,'story_ids':ids,'validation_sha256':hashlib.sha256(val.encode()).hexdigest(),'vocabulary':vocab,'protocol':'All validation stories; story-isolated evaluation; contiguous nonoverlapping context windows; each within-story next-character target once; separators excluded; context resets every context_length characters; no padding; bf16; fixed-checkpoint evaluation uncertainty only','total_stories':len(valid)}
(root/'manifest.json').write_text(json.dumps(manifest,indent=2))
outputs={}
for name in ['MHA','GQA','MQA','Collaborative MHA','GT-MHA residual']:
    start=time.perf_counter()
    ckpt=torch.load(root/(name+'.pt'),map_location='cpu',weights_only=False,mmap=True)
    cfg=ckpt['model_config']
    if name=='GT-MHA residual':
        assert cfg['num_base_heads']==4 and cfg['num_generators']==8
    ...
    for j,i in enumerate(ids):
        ...
        if (j+1)%1000==0:
            print(json.dumps({'model':name,'stories_done':j+1,'total_stories':len(ids),'seconds':time.perf_counter()-eval_start}),flush=True)
            (root/(name+'_partial.json')).write_text(json.dumps(rows))
    ...
    parts=[]
    for _ in range(20):
        indices=rng.integers(0,len(rows),(100,len(rows)))
        parts.append(losses[indices].sum(1)/counts[indices].sum(1))
    ...
    (root/'progress_summary.json').write_text(json.dumps(outputs,indent=2))
    print(json.dumps({name:{k:v for k,v in outputs[name].items() if k not in ['bootstrap_nll','config']}}),flush=True)
    del model; gc.collect(); torch.cuda.empty_cache()
reference=np.array(outputs['MHA']['bootstrap_nll'])
paired={}
for name,metrics in outputs.items():
    boot=np.array(metrics.pop('bootstrap_nll'))
    if name!='MHA':
        paired[name]={'nll_difference':metrics['nll']-outputs['MHA']['nll'], 'nll_ci95':np.quantile(boot-reference,[.025,.975]).tolist(),'ppl_difference':metrics['perplexity']-outputs['MHA']['perplexity'],'ppl_ci95':np.quantile(np.exp(boot)-np.exp(reference),[.025,.975]).tolist()}
outputs['paired_differences_vs_MHA']=paired
outputs['status']='FULL_EVALUATION_COMPLETE'
outputs['uncertainty']='2000 story-cluster percentile bootstrap replicates; fixed checkpoints; not training-seed variability; checkpoint selection used validation data'

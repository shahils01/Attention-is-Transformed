python experiments/train_vla.py   \
  --train_metas_path /scratch/shahils/openpi/datasets/Libero-XVLA-format/libero_meta.json    \
  --attention lgma_residual   \
  --generator_type full   \
  --device cuda:0 \
  --batch_size 128 \
  --grad_accum_steps 1 \
  --steps 500000 \
  --log_every 100 \
  --eval_every 1000 \
  --save_every 5000 \
  --output_dir /scratch/shahils/lgma_runs/libero_vla_lgma_multibase_b2_v1 \
  --diagnostic_every 1000 \
  --diagnostic_batches 2 \
  --wandb_project lgma_vla \
  --wandb_run_name libero_vla_lgma_multibase_b2_v1 \
  --wandb_group libero \
  --wandb_tags libero,lgma_vla,multibase,b2 \
  --induced_metric_diversity_weight 1e-3 \
  --metric_diversity_weight 1e-4 \
  --d_model 1024 \
  --num_layers 12 \
  --num_heads 16 \
  --num_base_heads 2 \
  --head_dim 64 \
  --base_dim 64 \
  --value_dim 64 \
  --num_generators 16 \
  --metric_beta 0.25 \
  --context_length 1024 \
  --dropout 0.1 \
  --lr 3e-4 \
  --weight_decay 0.01 \
  --precision bf16 \

  # --resume_checkpoint /scratch/shahils/lgma_runs/large_tinystories_lgma_multibase_b2/checkpoint_step_15000.pt \
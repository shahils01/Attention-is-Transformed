...
from lgma.seq2seq import WMT14Dataset, WMT14Transformer, collate_translation
...
def translation_loss(model, batch, pad_id):
    ...
    return F.cross_entropy(logits.flatten(0, 1), batch.target_ids.flatten(), ignore_index=pad_id)
...
def main():
    ...
    cfg = {'pad_id': ids['pad'], 'bos_id': ids['bos'], 'max_source_length': a.max_source_length, 'max_target_length': a.max_target_length, 'batch_size': a.batch_size}
    ...
    model = WMT14Transformer(vocab_size=manifest['tokenizer']['vocab_size'], d_model=a.d_model, ffn_dim=a.ffn_dim, num_encoder_layers=a.num_encoder_layers, num_decoder_layers=a.num_decoder_layers, num_heads=a.num_heads, head_dim=a.head_dim, attention_type=a.attention, dropout=a.dropout, max_source_length=a.max_source_length, max_target_length=a.max_target_length, pad_id=ids['pad'], **attention_kw).to(device)
    ...
    run = init_wandb_run(project=a.wandb_project, entity=a.wandb_entity, name=a.wandb_run_name, group=a.wandb_group, tags=a.wandb_tags, mode=a.wandb_mode, output_dir=a.output_dir, config={'args':vars(a), 'manifest':manifest, 'cross_attention':'standard_mha_shared_across_methods'}) if rank == 0 else None
    ...
    for step in range(start + 1, a.steps + 1):
        ...
        for micro in range(a.grad_accum_steps):
            indices = torch.randint(len(train), (a.batch_size,), generator=generator)
            ...
            with sync, torch.autocast(device_type=device.type, dtype=torch.bfloat16, enabled=a.precision == 'bf16' and device.type == 'cuda'):
                loss = translation_loss(model, batch, cfg['pad_id']) / a.grad_accum_steps
        ...
        if step % a.log_every == 0 or step == 1:
            ...
            if rank == 0:
                now=time.perf_counter(); payload={'step':step,'train_loss':loss.item(),'steps_per_second':a.log_every/max(now-last,1e-6),'tokens_per_step':a.batch_size*a.grad_accum_steps*world}; print(json.dumps(payload)); log_wandb(run,payload,step); last=now
